...
class MyPlayer:
    '''AB pruning. Depth = 4. Evaluation: difference between points, preevaluated board'''
    def __init__(self, my_color, opponent_color):
        ...
        self.depth = 4
        self.cur_move = []
        self.cell_costs = [[1000, -300, 100,  80,  80, 100,-300, 1000],
    ...
    def move(self, board):
        return self.minimax(copy.deepcopy(board), self.depth, -10000, 10000, my_turn = True)

    def minimax(self, board, cur_depth, alpha, beta, my_turn):
        if cur_depth == 0:
            score = self.count_score(board, self.my_color) - self.count_score(board, self.opponent_color)
            score += self.cell_costs[self.cur_move[0]][self.cur_move[1]]
            return score
        elif my_turn == True:
            # Tries to maximise 
            best_move = []
            ...
            for move in moves:
                if cur_depth == self.depth:
                    self.cur_move = move
                board_copy = self.repaint_the_board(copy.deepcopy(board), move[0], move[1], self.my_color, self.opponent_color)
            ...
            for move in moves:
                board_copy = self.repaint_the_board(copy.deepcopy(board), move[0], move[1], self.opponent_color, self.my_color)
                score = self.minimax(board_copy, cur_depth - 1, alpha, beta, my_turn = True)
    ...
    def repaint_the_board(self, board, y, x, p1_color, p2_color): 
